...
def _calc_ssim(f, im0, im1, scale=255):
...
def calc_ssim(type, im0, im1, scale=255, fn='msssim'):
    mdl = __import__(fn, fromlist=[type])
    cls = getattr(mdl, type)
    f = cls()
    # # assume that the data are *not* in chainer format (i.e. channels in front).
    if im0.ndim == 3:
        ...
        print('invalid dim')
        exit(-1)
    return _calc_ssim(f, im0, im1, scale=scale)
